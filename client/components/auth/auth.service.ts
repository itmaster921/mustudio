...
export function AuthService($location, $http, $cookies, $q, appConfig, Util, User) {
  ...
  var Auth = {
    ...
    login({email, password}, callback?: Function) {
      return $http.post('/auth/local', { email, password: md5(password) })
        .then(res => {
    ...
    /**
     * Create a new account (user + business)
     *
     * @param  {Account}   account     - user info
     * @param  {Function} callback - function(error, user)
     * @return {Promise}
     */
    createUser(account, callback?: Function) {
        var newAccount = angular.copy(account);
        if(newAccount['password']) newAccount['password'] = md5(newAccount.password);
      return User.save(newAccount,
        function(data) {
          //$cookies.put('token', data.token);
          //currentUser = User.get();
          return safeCb(callback)(null, account);
        },
        function(err) {
          Auth.logout();
